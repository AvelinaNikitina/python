...
class StoreMashines:
    ...
    def reception(self):
        ...
        # while True:
        try:
            self.unit = input(f'Введите наименование ')
            self.unit_p = int(input(f'Введите цену за ед '))
            self.unit_q = int(input(f'Введите количество '))
            unique = {'Модель устройства': self.unit, 'Цена за ед': self.unit_p, 'Количество': self.unit_q}
